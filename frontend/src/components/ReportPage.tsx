/**
 * ReportPage.tsx — Concise one-page research report on the OSHA Risk Model.
 * Designed to fit on a single printed page (use browser print / Save as PDF).
 */

// ── Pipeline SVG ──────────────────────────────────────────────────────────────

function PipelineDiagram() {
  // 6 boxes, each 118px wide, 18px gap between = (118+18)*6 - 18 = 798
  const BOX_W = 118
  const BOX_H = 44
  const GAP = 18
  const CY = 28
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${totalW} ${H}`} style={{ display: 'block' }}>
      ...
      {allNodes.map((n, i) => {
        const x = i * STEP
        const arrowX1 = x + BOX_W
        const arrowX2 = x + STEP - 2
        return (
          <g key={i}>
...
GradientBoostingRegressor (Heads 2, 3)
  ...
  return (
    <div className="report-page-print report-page" style={S.page}>
      {/* ── Header ── */}
      <div style={{ borderBottom: '2px solid #1d5a8e', paddingBottom: 6, marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          <span style={S.h1}>OSHA Manufacturer Compliance Risk Model</span>
          <span style={{ ...S.badge('#1d5a8e'), fontSize: 8 }}>Technical Report · April 2026</span>
        </div>
      ...
      {/* ── Pipeline ── */}
      <div style={S.sectionTitle}>1 · System Pipeline</div>
      <PipelineDiagram />

      {/* ── Two-column body ── */}
      <div className="report-grid-two" style={S.col2}>

        {/* LEFT col */}
        <div>
          {/* Feature set */}
          <div style={S.sectionTitle}>2 · Feature Set (49 dimensions)</div>
          <table style={S.table}>
            <thead>
              <tr>
            ...
            <tbody>
              {[
                ...
                <tr key={g}>
                  <td style={S.td}><strong>{g}</strong></td>
                  <td style={S.td} align="center">{c}</td>
                  <td style={{ ...S.td, color: '#64748b', fontSize: 8.2 }}>{e}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ ...S.bodyText, marginTop: 4, fontSize: 8.2, color: '#64748b' }}>
          ...
          {/* Composite formula */}
          <div style={S.sectionTitle}>3 · Composite Score Formula</div>
          <code style={S.code}>{compositeFormula}</code>
          <div style={{ ...S.bodyText, marginTop: 4, fontSize: 8.2, color: '#64748b' }}>
            Evidence ceiling applied post-scoring: fatality+willful → cap 70; ≤2 inspections → cap 50; 3–4 → cap 58; 5+ → no cap. Recommendation bands: <span style={S.badge('#059669')}>Recommend</span> &lt;30, <span style={S.badge('#d97706')}>Caution</span> 30–59, <span style={S.badge('#dc2626')}>Do Not Recommend</span> ≥60.
          </div>

          {/* Training setup */}
          <div style={S.sectionTitle}>4 · Training Setup</div>
          <table style={S.table}>
            <tbody>
              {[
                ...
                <tr key={k}>
                  <td style={{ ...S.td, fontWeight: 600, whiteSpace: 'nowrap' as const, fontSize: 8.5 }}>{k}</td>
                  <td style={{ ...S.td, color: '#374151', fontSize: 8.5 }}>{v}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div>
          ...
          {/* Head architecture */}
          <div style={S.sectionTitle}>5 · Model Architecture – 4 Prediction Heads</div>
          <table style={S.table}>
            <thead>
              <tr>
            ...
            <tbody>
              {[
                ...
                <tr key={h}>
                  <td style={{ ...S.td, fontWeight: 700, whiteSpace: 'nowrap' as const }}><span style={S.badge('#1d5a8e')}>{h}</span></td>
                  <td style={{ ...S.td, fontSize: 8.2 }}>{t}</td>
                  <td style={{ ...S.td, fontSize: 8.2, color: '#64748b' }}>{a}</td>
                  <td style={{ ...S.td, fontSize: 8.2 }}>{o}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ marginTop: 6 }}>
            <div style={{ ...S.bodyText, fontSize: 8.5, marginBottom: 3 }}>Composite head weight allocation:</div>
            <WeightChart />
          </div>

          {/* Hyperparams */}
          <div style={S.sectionTitle}>6 · Hyperparameters</div>
          <code style={{ ...S.code, fontSize: 7.8 }}>{hyperparams}</code>

          {/* Validation */}
          <div style={S.sectionTitle}>7 · Validation Metrics</div>
          <div style={{ ...S.bodyText, fontSize: 8.2, marginBottom: 4, color: '#64748b' }}>
          ...
          <table style={{ ...S.table, marginTop: 6 }}>
            ...
            <tbody>
              {[
                ...
                <tr key={m}>
                  <td style={{ ...S.td, fontWeight: 600, fontSize: 8.5 }}>{m}</td>
                  <td style={{ ...S.td, fontSize: 8.5 }}>{v}</td>
                  <td style={{ ...S.td, fontSize: 8.5, color: '#64748b' }}>{t}</td>
                  <td style={{ ...S.td, fontSize: 8, color: '#9ca3af' }}>{n}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ ...S.bodyText, fontSize: 8, color: '#9ca3af', marginTop: 4 }}>
