...
from src.search.grouped_search import (
    normalize_establishment_name, score_candidate_match,
...
test_cases = ["walmart", "walmart supercenter", "fastenal", "amazon", "parker hannifin", "3m"]
...
for query in test_cases:
    ...
    if result.top_group:
        g = result.top_group
        print(f"  TOP: {g.parent_name} | {g.total_facilities} facilities | {g.confidence_label} ({g.confidence:.2f})")
        print(f"       high={len(g.high_confidence)} med={len(g.medium_confidence)} low={len(g.low_confidence)}")
        for f in g.all_facilities[:5]:
            print(f"         raw=[{f.raw_name}] conf={f.confidence:.2f} ({f.confidence_label})")
        if g.total_facilities > 5:
            print(f"         ... and {g.total_facilities - 5} more")
    else:
        print("  TOP: (none)")
    for i, og in enumerate(result.other_groups):
        print(f"  OTHER[{i}]: {og.parent_name} | {og.total_facilities} facilities | {og.confidence_label} ({og.confidence:.2f})")
    if result.unmatched:
        print(f"  UNMATCHED: {result.unmatched[:5]}")

# Specific false-positive checks
print(f"\n{'='*60}")
print("False positive checks:")
for name in ["317733246 - Corp Inc", "317739632 - St Group Inc", "75808 - Al Group"]:
    norm = normalize_establishment_name(name)
    score = score_candidate_match("FASTENAL", norm)
    print(f"  '{name}' -> norm='{norm}' score={score:.2f}")
