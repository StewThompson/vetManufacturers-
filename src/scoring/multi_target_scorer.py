...
_DEFAULT_W1 = 0.578  # p_serious_wr_event
_DEFAULT_W2 = 0.256  # p_injury_event
_DEFAULT_W3 = 0.165  # p_extreme_penalty (P95 tier)
...
class MultiTargetRiskScorer:
    ...
    def fit_weights(
        ...
        if not self._is_fitted:
            return
        from src.scoring.calibration import _optimize_weights
        pred_val      = self.predict_batch(X_val)
        self._weights = _optimize_weights(pred_val, y_adv_val)
        
        # We must re-build the empirical CDF after changing weights!
        raw_vals = np.array([self._raw_composite(p) for p in pred_val])
        self._score_cdf = np.sort(raw_vals)
